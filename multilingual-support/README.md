...
- **Grammar-rule names** (`grammar.ruleName_<slug>`, one per `backend/seed_data.py` `GrammarRule`) are matched at render time via `slugifyRuleName(rule.name)` in `lib/locale.ts` — the slug must stay in sync with `add_language.py`'s `slugify()` if rule names in the DB ever change (both implement the same algorithm; see the comment on `slugifyRuleName`).
